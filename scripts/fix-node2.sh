#!/usr/bin/env bash
# fix-node2.sh - Run on NODE2 (172.16.15.37) only.
# Fixes: etcd v2, Patroni unit, starts Patroni.
# Usage: sudo bash scripts/fix-node2.sh
# Before first run: sed -i 's/\r$//' scripts/fix-node2.sh  (if copied from Windows)
